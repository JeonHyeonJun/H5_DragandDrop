...
public class VideoTest {
	...
	@RequestMapping(value="/contents/{content_id}/medias/video/{video_name:.+}", method= RequestMethod.GET) 
	public String getContentMediaVideo(@PathVariable("content_id")String content_id, 
			@PathVariable("video_name")String video_name,HttpServletRequest request, 
		...
		if( filename_seperate.length <= 1 ) { 
			// 확장자 에러  
			//String resultMsg = new MessageMaker().getErrorMessage("Wrong file name. You need to include expand file name"); 
		...
		File file = new File("/home/"+content_id+"/medias/video/"+ video_name); 
			if( ! file.exists() ) 
			...
			try{ 
				...
				do{ 
					...
					int len = randomFile.read(buf, 0, block); out.write(buf, 0, len); 
					partSize -= block; 
				} while(partSize > 0); 
			} catch(IOException e){ 
